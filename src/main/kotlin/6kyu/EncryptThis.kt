...
fun encryptThis(text:String): String{
    val myString = mutableListOf<String>()

    val myArray = text.split(" ")

    for(value in myArray) {
        if (value.length == 1) {
            myString += value[0].toInt().toString()
        } else if(value.length == 2) {
            myString += value[0].toInt().toString() + value[1]
        }
        else {
            val myCharArray = value.toCharArray()
            val temp = myCharArray[1]
            myCharArray[1] = myCharArray[myCharArray.size-1]
            myCharArray[myCharArray.size-1] = temp
            myString += myCharArray[0].toInt().toString() + String(myCharArray).drop(1)
